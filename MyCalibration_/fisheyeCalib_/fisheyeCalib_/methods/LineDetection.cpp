...
void LineDetection::loadImageXML(std::string filename)
{
    ...
    const char *psx = root->FirstChildElement("pixel_size_x")->GetText();
	pixel_size_x = atof(psx);

	const char *psy = root->FirstChildElement("pixel_size_y")->GetText();
	pixel_size_y = atof(psy);
        ...
        switch (pair->type) {
            case Four:
                for (int i = 0; i < 4; ++i) {
                    ...
                    img[i] = cv::imread(pair->filenames[i], cv::IMREAD_GRAYSCALE);
...
void LineDetection::saveParameters()
{
    ...
    tinyxml2::XMLElement *psx_elm = output.NewElement("pixel_size_x");
    psx_elm->SetText(pixel_size_x);
    output.RootElement()->InsertEndChild(psx_elm);

	tinyxml2::XMLElement *psy_elm = output.NewElement("pixel_size_y");
	psy_elm->SetText(pixel_size_y);
	output.RootElement()->InsertEndChild(psy_elm);
...
void LineDetection::saveTwoSetOfLines(std::vector<std::vector<cv::Point2i> >& first, std::vector<std::vector<cv::Point2i> >& second)
{
    ...
    for (auto &lines : first) { // First lines
        ...
        for (auto &point : lines) {
            ...
            sprintf_s(str, "%f %f", point.x/(float)unit, point.y/(float)unit);
            p->SetText(str);
...
void LineDetection::removeNoiseLine(std::vector<std::vector<cv::Point2i>>& lines, bool isHorizon)
{
	...
	{
		for (int i = 0; i < lines.size(); ++i) {
			cv::Rect rect_ = cv::boundingRect(lines[i]);
...
std::vector<std::vector<std::vector<cv::Point2i> > > LineDetection::loadEdgeXML(std::string filename)
{
    ...
    pixel_size_x = atof(root->FirstChildElement("pixel_size_x")->GetText());
	pixel_size_y = atof(root->FirstChildElement("pixel_size_y")->GetText());
