...
cv::Point3d StereographicProjection::calcDu()
{
    if (r != 0) {
        ...
        tu *= - pow(cos(theta/2.0), 2) * (point.x-center.x) * px_size.x * px_size.x / (r * f);
        mu += part * tu;
        return mu;
...
cv::Point3d StereographicProjection::calcDv()
{
    if (r != 0) {
        ...
        tv *= - pow(cos(theta/2.0), 2) * (point.y-center.y) * px_size.y * px_size.y / (r * f);
        mv += part * tv;
        return mv;
...
cv::Point3d StereographicProjection::calcDpy()
{
	if (r != 0) {
		...
		tpy *= pow(cos(theta / 2.0), 2) * pow(point.y - center.y, 2) * px_size.y / (r * f);
		mpy += part * tpy;
		return mpy;
...
std::vector<cv::Point3d> StereographicProjection::calcDak()
{
    ...
    if (r != 0) {
        cv::Point3d m;
        m = part * (cos(theta / 2.0) * cos(theta / 2.0) *f0 / f);
        for(int i=0; i<a.size(); ++i) {
